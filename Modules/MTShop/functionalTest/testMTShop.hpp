...
#ifndef TEST_MTSHOP_HPP
#define TEST_MTSHOP_HPP
...
class testMTShop : public clsBaseTest
{
    Q_OBJECT

public:
    testMTShop(const QString &_dbPrefix) : clsBaseTest(_dbPrefix) { ; }
...
//        DAC.execQuery("", QString("UPDATE %1AAA.tblUser SET usrStatus='R' WHERE usrEmail IN(?,?)").arg(this->DBPrefix), { UT_UserEmail, UT_AdminUserEmail });
        ...
        if (this->CouponID > 0) {
            try {
                QVariant Result = callAdminAPI(
                    ...
                    QString("MTShop/AccountCoupons/%1").arg(this->CouponID.toString())
                );
        ...
        //                QVariant Result = callAdminAPI(
        //                    DELETE,
        //                    QString("MTShop/PaymentGateways/%1").arg(paymentGatewayID.toString())
        //                );
        ...
        if (this->BannerSaleableID > 0) {
            try {
                QVariant Result = callAdminAPI(
                    RESTClientHelper::DELETE,
                    QString("MTShop/AccountSaleables/%1").arg(this->BannerSaleableID.toString())
                );
        ...
        if (this->BannerProductID > 0) {
            try {
                QVariant Result = callAdminAPI(
                    RESTClientHelper::DELETE,
                    QString("MTShop/AccountProducts/%1").arg(this->BannerProductID.toString())
                );
        ...
        if (this->LocationID > 0) {
            try {
                QVariant Result = callAdminAPI(
                    RESTClientHelper::DELETE,
                    QString("MTShop/Locations/%1").arg(this->LocationID.toString())
                );
    ...
    void updateProduct_banner() {
        QT_TRY {
            auto Result = callAdminAPI(
                ...
                QString("MTShop/AccountProducts/") + this->BannerProductID.toString(),
                {},
                {
