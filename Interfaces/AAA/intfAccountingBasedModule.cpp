...
#ifdef QT_DEBUG
void checkPreVoucherSanity(stuPreVoucher_v1 _preVoucher) {
    if (_preVoucher.Items.isEmpty())
        return;

//    stuVoucherItem PreVoucherItem = _preVoucher.Items[0];
//    QString oldSign = PreVoucherItem.Sign;
//    PreVoucherItem.Sign.clear();
//    auto newsign = QString(voucherSign(QJsonDocument(PreVoucherItem.toJson()).toJson()).toBase64());

//    PreVoucherItem = _preVoucher.Items[1];
//    oldSign = PreVoucherItem.Sign;
//    PreVoucherItem.Sign.clear();
//    newsign = QString(voucherSign(QJsonDocument(PreVoucherItem.toJson()).toJson()).toBase64());


    QString Sign = _preVoucher.Sign;
    _preVoucher.Sign.clear();

    auto js = _preVoucher.toJson();
    auto js2 = QJsonDocument(js).toJson();
    auto vsg = voucherSign(js2);
    auto sg = QString(vsg.toBase64());
...
Targoman::API::AAA::stuPreVoucher_v1 intfAccountingBasedModule::apiPOSTremoveBasketItem_v1(
    APICALLBOOM_TYPE_JWT_IMPL &APICALLBOOM_PARAM,
