...
#define INTERNAL_TAPI_ADD_TYPE_SPECIALFROMVARIANT(_baseType, _typeName, _fromVariant) \
    ...
    inline void setFromVariant(QSharedPointer<_typeName>& _storage, const QVariant& _val){ \
...
#define INTERNAL_NULLABLE_FROM_JSONVALUE_TO_TYPE(_type, _value) \
    [](auto v)->QSharedPointer<_type> { \
        if (v.isNull()) \
            return nullptr; \
        auto Storage = QSharedPointer<quint32>(); \
        *Storage = v.toDouble(); \
        return Storage; \
    } (_value)
...
namespace TAPI { \
    ...
    inline void setFromVariant(QSharedPointer<_enum::Type>& _storage, const QVariant& _val){ \
