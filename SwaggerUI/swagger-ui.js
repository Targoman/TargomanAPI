...
this["Handlebars"]["templates"]["sidebar_item"] = Handlebars.template({"compiler":[6,">= 2.0.0-beta.1"],"main":function(depth0,helpers,partials,data) {
  var helper, functionType="function", helperMissing=helpers.helperMissing, escapeExpression=this.escapeExpression;
  var relativePath = escapeExpression(((helper = (helper = helpers.relativePath || (depth0 != null ? depth0.relativePath : depth0)) != null ? helper : helperMissing),(typeof helper === functionType ? helper.call(depth0, {"name":"summary","hash":{},"data":data}) : helper)))
    + "\n";
  var httpMethod = escapeExpression(((helper = (helper = helpers.httpMethod || (depth0 != null ? depth0.httpMethod : depth0)) != null ? helper : helperMissing),(typeof helper === functionType ? helper.call(depth0, {"name":"summary","hash":{},"data":data}) : helper)))
  + "\n";
  ...
  switch(httpMethod) {
    case 'GET':
      class_ = 'operation get';
      break;
    case 'POST':
      class_ = 'operation post';
      break;
    case 'DELETE':
      class_ = 'operation delete';
      break;
    case 'PATCH':
      class_ = 'operation patch';
      break;
    case 'PUT':
        class_ = 'operation post';
        break;
...
//      console.log("${httpMethod}" !=? POST);
  ...
  return '<span class="swagger-section sidebar-method-icon"><span class="heading"><span class="' + class_ + '"><span class="http_method"><a href="#">' + httpMethod + '</a></span><span style="display:inline-block;vertical-align: bottom">&nbsp;' + relativePath + '</span></span></span></span>';
},"useData":true});
...
SwaggerClient.prototype.buildFromSpec = function (response) {
  ...
  _.forEach(response.paths, function (pathObj, path) {
    ...
    _.forEach(supportedOperationMethods, function (method) {
      ...
      if (_.isUndefined(operation.tags) || !_.isArray(operation.tags) || operation.tags.length === 0) {
        operation.tags = [ 'default' ];
      }

      var tags = path.split('/').filter(e => e.length > 0).slice(0, 1);
...
SwaggerUi.Views.SidebarHeaderView = Backbone.View.extend({
  ...
  render: function () {
    ...
    for (var i = 0; i < this.model.operationsArray.length; i++) {
      var item = this.model.operationsArray[i];
      console.log(item);
      var effectivePathPartsLength = 0;
      if(level < 1) {
        var pathParts = item.path.split('/').filter(e => e.length > 0).slice(level + 1);
        var effectivePathPartsLength = pathParts.length;
        if(pathParts.length > 0 && /\{.*?\}/.test(pathParts[pathParts.length - 1]))
          --effectivePathPartsLength;
        item.relativePath = '/' + pathParts.slice(1).join('/');
      }
      if(effectivePathPartsLength > 0) {
        if(!resources.hasOwnProperty(pathParts[0])) {
          resources[pathParts[0]] = _.clone(this.model);
          resources[pathParts[0]].name = pathParts[0];
          resources[pathParts[0]].level = level + 1;
          resources[pathParts[0]].operationsArray = [];
        }
        resources[pathParts[0]].operationsArray.push(item);
      } else {
        items.push(item);
      }
    }
    ...
    for (var key in resources) {
      this.addSidebarHeaderView(resources[key], index);
      ++index;
    }

    for (var i = 0; i < items.length; i++) {
      var item = items[i].operation;
      item.nickname = items[i].nickname;
      item.parentId = this.model.operation.parentId;
      item.httpMethod = items[i].method ? items[i].method.toUpperCase() : items[i].method;
      item.relativePath = items[i].relativePath;
  ...
  addSidebarHeaderView: function(resource, i) {
    var sidebarView = new SwaggerUi.Views.SidebarHeaderView({
      model: resource,
      tagName: 'div',
      className: function () {
      ...
      attributes: {
        "data-resource": this.options.attributes["data-resource"] +  '_resource_' + resource.name,
        "label": resource.name
      },
      router: this.router,
      swaggerOptions: this.options.swaggerOptions
...
SwaggerUi.Views.SidebarItemView = Backbone.View.extend({
  ...
  render: function () {
    ...
    $(this.el).css({ padding: '9px'});
    $('a', this.el).on('click', function() { el.click(); return false; });
    return this;
  }
